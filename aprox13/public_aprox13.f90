...
! store the old abundances
       ylast(ionbeg:ionend) = y(ionbeg:ionend)
...
! screen print
       if (iprint_screen .eq. 1) then
        ...
        call azbar(xdum(ionbeg),aion(ionbeg),zion(ionbeg),wion(ionbeg),ionmax, &
                   zwork1(ionbeg),abar,zbar,wbar,ye,xcess)
...
! transfer the info stored in xsum and zsum from the update2 call
      ...
      if (trho_hist) then
       ...
       xin(ionbeg:ionend) = xsum(ionbeg:ionend)
       tstart     = zwork1(1)
       tstep      = zwork1(2)
       zye        = zwork1(3)
      end if
...
! put it in nse
       ...
       igues = 1
       call nse(tin,din,zye,igues,1,1,xin(ionbeg),xmun,xmup,0)
...
!      write(6,02) (ionam(i),xin(i), i=ionbeg,ionend)
!      read(5,*)
...
! get the ye of the initial compositon
        call azbar(xin(ionbeg),aion(ionbeg),zion(ionbeg),wion(ionbeg),ionmax, &
                   zwork1(ionbeg),abar,zbar,wbar,ye_orig,xcess)
...
! get the abundance variables for the final mixture
        call azbar(xin(ionbeg),aion(ionbeg),zion(ionbeg),wion(ionbeg),ionmax, &
                   zwork1(1),abar,zbar,wbar,ye_orig,xcess)
...
! largest mass fractions
        call indexx(ionend,xin(ionbeg),izwork1(ionbeg))
        j = min(20,ionmax)
        k = max(ionend-19,1)
...
! figure delta on the top 20 nse mass fractions
       ...
       sum = 0.0d0
       kb  = 0
       do j = ionend, max(ionbeg,ionend-19), -1
...
!---------------------------------------------------------------------
      ...
      include 'implno.dek'
      double precision tt,temp,den
      stop 'should not be in update2.f with nothing in the routine'
      return
      end
...
! write out the biggest mass fractions
 ...
 20   continue
       ...
       write(6,02) (ionam(izwork1(i)+ioff),xout(izwork1(i)+ioff), i=ionend,k,-1)
       ...
       write(41,02) (ionam(izwork1(i)+ioff),xout(izwork1(i)+ioff), i=ionend,k,-1)
       if (iprot .ne. 0 .and. ineut .ne. 0) then
