...
int main()
{
	...
	double density, energy, abar, zbar, temperature, pressure,pin;
	int i, j, k;
	...
	{
		density+=1.0e6;
		Fortran2(wrapper_helmeos)(&npart, &density, &energy, &abar, &zbar, &temperature, &pin); 
	}
	
	printf("density\t\t= %2.3e g/cc\n",density);
	printf("energy\t\t= %2.3e erg/g\n",energy);
	printf("temperature\t= %2.3e K\n",temperature);
	printf("pin\t= %2.3e g/cm/s^2\n",pin);
